...
#include "./src/loaduser.h"
#include "./src/loadkicau.h"
#include "./src/loadbalasan.h"
#include "./src/loaddraf.h"
#include "./src/loadutas.h"
#include <ctype.h>
...
void ReadUserData(const char *filename, UserData *userData) {
    ...
    userData->banyak_pengguna = WordToInt(currentWord);
    ...
    for (int i = 0; i < userData->banyak_pengguna; i++) {
        for (int j = 0; j < userData->banyak_pengguna; j++) {
            ...
            userData->adjacencyMatrix[i][j] = WordToInt(currentWord);
    ...
    for (int i = 0; i < banyak_permintaan; i++) {
        ADVWORD(); currentWord.TabWord[currentWord.Length] = '\0';
        int senderID = WordToInt(currentWord);
        
        ADVWORD(); currentWord.TabWord[currentWord.Length] = '\0';
        int receiverID = WordToInt(currentWord);
        
        ADVWORD(); currentWord.TabWord[currentWord.Length] = '\0';
        int friendCount = WordToInt(currentWord);
...
void ReadKicauData(char* filename, kicauan* kicau) {
    ...
    kicau->jumlah_kicau = WordToInt(currentWord);
    printf("Jumlah kicauan: %d\n", kicau->jumlah_kicau);

    // Membaca kicauan
    for (int i = 0; i < kicau->jumlah_kicau; i++) {
        printf("\nReading kicau %d\n", i);

        // Membaca id kicauan
        ADVLINE(); currentWord.TabWord[currentWord.Length] = '\0';
        kicau->id = WordToInt(currentWord);
        printf("ID: %d\n", kicau->id);
        ...
        ADVLINE(); currentWord.TabWord[currentWord.Length] = '\0';
        kicau->Likes = WordToInt(currentWord);
        printf("Likes: %d\n", kicau->Likes);
...
void ReadBalasanData(char* filename, Balasan* balasan) {
    ...
    STARTFILEWORD(filename);
    balasan->kicauBalas = WordToInt(currentWord);
    printf("Jumlah balasan: %d\n", balasan->kicauBalas);

    // Membaca id kicauan yang memiliki balasan
    ADVLINE(); currentWord.TabWord[currentWord.Length] = '\0';
    balasan->idKicau = WordToInt(currentWord);
    // printf("ID: %d\n", balasan->idKicau);

    // Membaca banyak balasan yang diterima
    ADVLINE(); currentWord.TabWord[currentWord.Length] = '\0';
    balasan->banyakBalasan = WordToInt(currentWord);
    ...
    for (int i = 0; i < balasan->banyakBalasan; i++) {
        ...
        // Membaca id parent
        ADVWORD(); currentWord.TabWord[currentWord.Length] = '\0';
        balasan->idParent = WordToInt(currentWord);
        // printf("ID parent: %d\n", balasan->idParent);

        // Membaca id balasan
        ADVWORD(); currentWord.TabWord[currentWord.Length] = '\0';
        balasan->idBalasan = WordToInt(currentWord);
        // printf("ID balasan: %d\n", balasan->idBalasan);
...
void ReadDrafData(const char *filename, ListDraf *listDraf) {
    ...
    listDraf->banyak_pengguna = WordToInt(currentWord);
    ...
    for (int i = 0; i < listDraf->banyak_pengguna; i++) {
        ...
        while (1) {
            ...
            if (isdigit(currentWord.TabWord[0])) {
                listDraf->users[i].banyak_draf = WordToInt(currentWord);
...
void ReadUtasData(const char *filename, DataUtas *dataUtas) {
    ...
    dataUtas->banyak_kicauan = WordToInt(currentWord);
    ...
    for (int i = 0; i < dataUtas->banyak_kicauan; i++) {
        ...
        ADVLINE();
        currentWord.TabWord[currentWord.Length] = '\0';
        dataUtas->kicauans[i].id_kicauan = WordToInt(currentWord);
        printf("ID kicauan %d: %d\n", i + 1, dataUtas->kicauans[i].id_kicauan);

        // Read the number of utas for this kicauan
        ADVLINE();
        currentWord.TabWord[currentWord.Length] = '\0';
        dataUtas->kicauans[i].banyak_utas = WordToInt(currentWord);
